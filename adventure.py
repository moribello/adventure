...
def forest(monster):
    ...
    """
    choices = {"\033[1;32mPick flowers\033[0m" : ["pick", "flowers"], "Go to the \033[1;32mshed\033[0m" :
               ["shed"], "\033[1;32mReturn\033[0m to the \033[1;32mclearing\033[0m" : ["return", "clearing"]}
    ...
    if "fought monster" not in actions:
        ...
        choices.update({"\033[1;32mRun away!\033[0m" : ["run", "run away"], "\033[1;32mFight!\033[0m" : ["fight"]})
    action = valid_input(choices)
    if action == "Pick flowers":
        ...
        if "sword" in inventory:
            exclaimations = ["\033[0;31mPow!\033[0m", "\033[1;33mBiff!\033[0m", "\033[0;31mBang!\033[0m", "\033[1;33mKlang!\033[0m", "\033[0;36mSwish!\033[0m"]
...
def house_kitchen():
    ...
    """
    choices = {"Go to \033[1;32mliving room\033[0m" : ["living room", "living"]}
    if "vase" in inventory and "flowers" in inventory:
        if "flowers in vase" not in inventory:
            choices.update({"Put \033[1;32mflowers\033[0m in \033[1;32mvase\033[0m" : ["flowers", "vase"]})
    ...
    if "make coffee" not in actions:
        choices.update({"Make \033[1;32mcoffee\033[0m" : ["make coffee", "coffee"]})
    if "make coffee" in actions and "cup of coffee" not in inventory:
        choices.update({"Pick up cup of \033[1;32mcoffee\033[0m" : ["cup of coffee", "coffee"]})
        ...
        if "vase" in inventory or "flowers in vase" in inventory:
            if "tray" not in inventory:
                ...
                if "pick up coffee" not in choices:
                    choices.update({"pick up \033[1;32mcoffee\033[0m" : ["coffee"]})
            else:
