...
def clearing():
    ...
    choices = {"Go to the farmhouse" : ["farm", "house", "farmhouse"],
               "Go to the shed" : ["shed"], "Enter the forest" : ["forest"]}
    ...
    if "clean up bottles" not in actions:
        ...
        choices.update({"Clean up bottles" : ["clean up", "bottles"]})
    ...
    if "note" not in inventory:
        ...
        choices.update({"Pick up the note" : ["note"]})
...
def forest(monster):
    ...
    """
    choices = {"Pick flowers" : ["pick", "flowers"], "Go to the shed" :
               ["shed"], "Return to the clearing" : ["return", "clearing"]}
...
def shed():
    ...
    """
    choices = {"Leave the shed" : ["leave", "shed"]}
    ...
    if "keys" not in inventory:
        ...
        choices.update({"Pick up the keys" : ["keys", "pick up keys"]})
    if "sword" not in inventory:
        ...
        choices.update({"Pick up the sword" : ["sword", "pick up sword"]})
    action = valid_input(choices)
...
def house_entry():
    ...
    choices = {"Leave house" : ["leave"], "Enter living room" : ["enter", "living"], "Look in mirror" : ["look", "mirror"]}
...
def house_livingroom():
    ...
    if "vase" not in inventory and "flowers in vase" not in inventory:
        print_pause("An interesting-looking vase sits on a bureau against the"
                    " right wall.")
        choices.update({"Pick up the vase" : ["vase"]})
    if "tray" not in inventory:
        print_pause("A tea tray sits on the coffee table.")
        choices.update({"Pick up the tray" : ["tray", "tea tray"]})
    action = valid_input(choices)
        ...
        if "vase" in inventory or "flowers in vase" in inventory:
            if "tray" not in inventory:
                ...
                if "pick up coffee" not in choices:
                    choices.update({"pick up coffee" : ["coffee"]})
            else:
